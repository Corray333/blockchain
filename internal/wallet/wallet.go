...
func InitializeWallet(recoveryPhrase string) error {
	seed, err := GenerateSecretNumberBySeedPhrase(recoveryPhrase)
	if err != nil {
		slog.Error("error while generating private and public keys: " + err.Error())
		return errors.New("error while generating private and public keys: " + err.Error())
	}
	...
	if !ok {
		...
		return errors.New("error while generating private and public keys")
	}
...
func GenerateSecretNumberBySeedPhrase(phrase string) (string, error) {
	...
	if err != nil {
		...
		return "", fmt.Errorf("error while reading wordlist: %s", err.Error())
	}
	...
	for _, v := range recPhrase {
		...
		if i == -1 {
			...
			return "", fmt.Errorf("wrong recovery phrase")
		}
